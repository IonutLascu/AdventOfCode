...
{
    string row;
    vector<pair<int, int>> partOne { {3, 1} };
    vector<pair<int, int>> partTwo{ {1, 1}, {3, 1}, {5, 1}, {7, 1}, {1, 2} };
    vector<string> routes{};
    while (!is.eof()) {
        is >> row;
        routes.push_back(row);
    }
    std::cout << countTrees(routes, partOne.front()) << "\n";

    long long int result = 1;
    for (const auto i : partTwo) {
        result *= countTrees(routes, i);
    }
